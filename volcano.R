...
VolcanoPlot = function(df, logFC_thresh, padj_thresh, contrast_name){
  ...
  #lim_logFC = round(max(abs(min(mydata$log2FoldChange)), abs(max(mydata$log2FoldChange)))*1.2)
  ...
  g = ggplot(data=mydata, aes(x=log2FoldChange, y=-log10(padj),label=Gene, colour=threshold)) +
    ...
    xlab("log2 fold change") + ylab("-log10 padj") + scale_colour_manual(values = c("A"= "darkgreen", "B"="red",  "C"= "black")) +geom_hline(yintercept = -log10(padj_thresh), colour='blue') + geom_vline(xintercept = c(logFC_thresh, -logFC_thresh), colour='blue') + ggtitle(paste(contrast_name,'\nVolcano plot for abs(LogFC) >', logFC_thresh,'and padj <',padj_thresh))
  return(g)
